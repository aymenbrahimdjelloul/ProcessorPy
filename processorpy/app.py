...
class InfoDisplayWidget(QWidget):
    """Widget for displaying CPU information using absolute positioning"""
    ...
    def update_sensor_value(self, key: str, value: str) -> None:
        """Update a sensor value if the label exists"""

        if key in self.sensor_labels:
            self.sensor_labels[key].setText(f"{key}: {value}")

    def add_sensor_item(self, key: str,
                        value: str,
                        x: int = 220,
                        y: int = None,
                        font: QFont = None,
        ...
        """Add a sensor item using absolute positioning"""

        formatted_text: str = f"{key} : {value}"
        label = QLabel(formatted_text, self)

        # Use priority style for voltage and usage
        if priority:
            label.setStyleSheet(StyleManager.get_priority_sensor_style())
        else:
            label.setStyleSheet(StyleManager.get_info_label_style())
        ...
        for key, value in cpu_info.items():
            if key != "name":  # Skip name as it's in the button
                self.info_display.add_info_item(
                    key, str(value), 10, None,  # x=10 for left column, y=None for auto-positioning
                    self.font_manager.get_font(10)
                )
...
class Worker(QThread):
    ...
    def __init__(self, cpu_obj) -> None:
        ...
        self.cpu_obj: Processor = cpu_obj
